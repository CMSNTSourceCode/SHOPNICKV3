      ...
      <div class="card">
        <div class="card-body grid grid-cols-1 gap-x-6 p-6 md:grid-cols-2">
          ...
          <div class="flex flex-col justify-center space-y-3 text-center">
            <div class="mb-2">
              <h1 class="mb-1 text-[20px] md:text-[30px] ">{{ __('Chi Tiết Tài khoản') }} #<span class="copy cursor-pointer text-red-600" data-clipboard-text="{{ $item->code }}">{{ $item->code }}</span></h1>
            ...
            <div>
              ...
              @else
                <h2 class="text-[18px] md:text-[24px] text-primary">{{ __('Giá') }}: {{ $item->price_str }}</h2>
              @endif
            </div>
              ...
              @if ($item->group->game_type === 'thue-dot-kich')
                <div class="mt-5">
                  <a href="{{ Helper::getConfig('contact_info')['facebook'] ?? '#!' }}" target="_blank" class="btn btn-primary relative bg-cover bg-center bg-no-repeat "
                    ...
                    <i class="fas fa-share me-2"></i> {{ __('Liên Hệ Admin') }}</a>
                </div>
              @else
                @if ($item->is_sold === true)
                  <button class="btn mt-5 btn-success relative bg-cover bg-center bg-no-repeat cursor-wait" style="background-image: url(/images/matrix.jpg)" disabled>
                    <i class="fas fa-shopping-cart me-2"></i> {{ __('Đã Được Bán') }}
                  </button>
                @else
                  <button class="btn mt-5 btn-primary relative bg-cover bg-center bg-no-repeat " style="background-image: url(/images/matrix.jpg)" id="btnBuy" onclick="confirmBuy()">
                    <i class="fas fa-credit-card me-2"></i> {{ __('Thanh Toán') }}</button>
      ...
      <div class="card">
        <div class="card-body flex flex-col p-6">
          <header class="-mx-6 mb-5 flex items-center border-b border-slate-100 px-6 pb-5 dark:border-slate-700">
            <div class="flex-1">
              <div class="card-title text-slate-900 dark:text-white">{{ __('Chi tiết sản phẩm') }} - {{ $item->name }} : </div>
            </div>
          </header>
      ...
      <script>
        ...
        const buyItem = async (item) => {
          ...
          try {
            ...
            Swal.fire('Thành Công', `${result.message}<br /><br />Tài Khoản: ${username}<br />Mật Khẩu: ${password}<br />Cookie/2FA: ${extra_data}`, 'success').then(() => {
