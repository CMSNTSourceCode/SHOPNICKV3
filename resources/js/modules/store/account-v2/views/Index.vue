    ...
    const { value: oQuantity } = await Swal.fire({
      ...
      inputValidator: (value) => {
        ...
        if (parseInt(value) > item.is_bulk) {
          return __t('Số lượng tài khoản cần mua không được lớn hơn') + ' ' + item.is_bulk
        }
      },
      inputAttributes: {
        min: 1,
        ...
        <a-col :xs="24" :md="8" :lg="6" v-for="item in products" :key="item.id" class="mb-2 mt-2 cursor-pointer">
          <a-badge-ribbon color="red" :text="`-${item.discount}%`" :class="{ hidden: item.discount <= 0 }" placement="start">
            <a-badge-ribbon color="pink" :text="`MS: ${item.code}`" placement="end">
              <div class="border border-primary rounded-lg p-[1px] bg-white dark:bg-gray-800">
                <a-image :src="item.image" :alt="item.name" width="100%" class="rounded-t-lg sm:!h-[200px] lg:!h-[170px]" />
                <div class="text-center text-lg font-bold bg-primary text-white" style="margin-top: -6px">
                  {{ item.name }}
                </div>
                <div class="p-3">
                  ...
                  <div class="text-center grid grid-cols-2 gap-3">
                    <div class="col-span-2">
                      <div class="border border-red-500 rounded-lg p-1 font-bold dark:text-white">
                        {{ __t('Còn lại') }}
                        <span class="text-green-600">{{ item.amount }}</span>
                        {{ __t('tài khoản') }}
                      </div>
                      <div v-if="item.is_bulk > 1" class="border border-red-500 rounded-lg p-1 font-bold dark:text-white mt-2">
                        {{ __t('Được mua') }} <span class="text-red-500">{{ item.is_bulk }}</span> {{ __t('tài khoản / lần') }}
                      </div>
                    </div>
                    <div>
                      <a-button v-if="item.discount !== 0" type="dashed" @click="buyItem(item)" class="dark:text-white mb-1"><del>{{ formatCurrency(item.price) }}</del></a-button>
                      <a-button type="dashed" danger @click="buyItem(item)" block>{{ formatCurrency(item.price - (item.price * item.discount) / 100) }}</a-button>
                    </div>
                    <div>
                      <div v-if="item.discount !== 0" style="padding: 18px"></div>
